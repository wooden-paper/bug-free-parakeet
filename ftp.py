...
class MyFTP:
    ...
    """
    def download_local(self, filename, file):
        return self.ftp.retrbinary('RETR %s' % filename, file.write)
    ...
    def send_bytes(self):
        while not self.finished.is_set():
            print(self.ftp.timeout())
            try:
                yield self.bytes.get(timeout=self.ftp.timeout)
                self.bytes.task_done()
            except Empty:

                self.finished.wait(self.ftp.timeout)
        self.worker.join()
